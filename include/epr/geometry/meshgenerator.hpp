...
namespace epr::geometry::mesh {
    ...
    static epr::geometry::LocalTriangle generate_triangle(epr::spatial::Transform3 origin ,epr::math::Vector3 vertex_position[], std::string texture) {
        epr::geometry::LocalTriangle temp(texture);
    ...
    static std::vector <epr::geometry::LocalTriangle> generate_quad(epr::spatial::Transform3 origin, std::string texture) {
        ...
        texture[texture.find(';') + 1] = 'X';

        temp_mesh.push_back(generate_triangle(origin, triangle_v1_vertex, texture));

        texture[texture.find(';') + 1] = 'F';

        temp_mesh.push_back(generate_triangle(origin, triangle_v2_vertex, texture));
